...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePosition in gameState.getPossibleHouses():
        ...
        for walls in gameState.getHouseWalls(housePosition):
            ...
            if housePosition == bottomLeftPos:
                houseVal = BOTTOM_LEFT_VAL
            elif housePosition == topLeftPos:
                houseVal = TOP_LEFT_VAL
            elif housePosition == bottomRightPos:
                houseVal = BOTTOM_RIGHT_VAL
            elif housePosition == topRightPos:
                houseVal = TOP_RIGHT_VAL
            else:
                houseVal = None
            
            #factor with dependencies on ghost and food house
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                print(assignment)
                ghostHouseVal,foodHouseVal, color = assignment[GHOST_HOUSE_VAR], assignment[FOOD_HOUSE_VAR], assignment[obsVar]

                if houseVal == ghostHouseVal or houseVal == foodHouseVal:
                    ...
                    if color == "red":
                        ...
                        obsFactor.setProbability(assignment, 1 - red)
                    elif color == "none":
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    foodHouseDistribution = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    
    bestPos = None
    bestProb = float('-inf')
    
    for assignment in foodHouseDistribution.getAllPossibleAssignmentDicts():
